...
### T1 的硬约束（2026-09-21）
- ⛔ **教务系统不提供「节次 → 钟点」**：课表只给 `Sjd` 拼接码与 `Jieci`/`SjdText` 块名（`上午 3-4节`）。
  已 grep 全部已抓 JS（课表/教室课表/考试安排）找钟点 → **零命中**；主菜单 20 项无「作息时间」入口；
  项目代码里也没有这张表。→ **课表 .ics 与今日通知都写不出 `DTSTART`，被它卡住。**
  原则：**不内置猜的默认值**（猜错 = 导出的日历静默全错，且用户不会想到去改），
  改成「首次使用引导确认一次 + 表单可编辑」。
  **考试 .ics 不受影响** —— `KsSj` 自带 `晚上7：00-9：00`（⚠️ **全角冒号**），P0 就能做。
- 导航 5 → 4：选课与抢课合并为一项。`RushScreen`/`RushViewModel` **只降级为内层视图、不改行为**
  （它订阅 `RushStore` 单例，位置变了订阅不变）。内层切换用**名称**存，不用序号。
- **既有缺陷**：`RushScheduler` 没有 `BOOT_COMPLETED` 接收器 → **重启后闹钟丢失，定时抢课静默失效**。
  待修（今日课程提醒共用同一接收器）。
- 方案见 `docs/T1功能实施大纲.md`（阶段划分 P0 导航合并+考试页 → P1 学籍/导师 → P2 课表 .ics+通知）
