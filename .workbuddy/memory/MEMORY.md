...
- 课表 .ics 导出仍**阻塞**（缺节次钟点），新线索 `ViewKebiao.aspx`（T2 §6.1，待验）。明确不做：评教 · 免修补修 ·
...
- **写静态脚本**：扫描器多吐一个字符错的是用它的人（「包含」察觉不到、「精确比较」必误报）；剥块注释要把换行补
  回来，否则行号全前移；**探针抓不住的防御就是多余的防御** → 删掉并写清理由。
...
## 协作方式（2026-09-22 起）
**真机测试默认由用户执行**；经用户拍板的**预跑批次例外**（判据留进验收清单，变异只在预跑批次内做，跑完还原并
重建正式包）。自己做的：只读检查、离线构建与编译、进程内自检、纯逻辑对账。
