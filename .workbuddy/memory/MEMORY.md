...
- **周次/开学日期**：服务端不提供，只能靠一个**锚点（第一周周一）**。
  ⚠️ **锚点只对「有补考的学生」在学期初能自动推出来**（2026-09-21 实测）：学期初的考试安排里
  只有补考（期末考要到期末才排）。当前账号有 3 条补考所以自测正常；没挂科的同学拿到
  `Data:[]`，推不出。**推不出是学期初的常态，不是边缘情况。** 详见
...
- **锚点体系（2026-09-21 起，提交 1bddf7f）** —— 教务内没有第二个日期源（7 处候选逐个实测排除），
  所以「不知道第几周」这件事只能靠**反向校准**兜底：
  - 落盘 `TermAnchor` = `2026-08-31|manual|时间戳`（**单条**，不按学期分组 —— 分组会引入
    「当前学期是哪个」这个得同步的隐藏状态）。脏值/认不出的来源一律 decode 成 null，不猜日期。
  ...
  - **`anchorFitsTerm`**：锚点是单条的，而学期选择器能翻到十几年前 —— 用当前学期的锚点渲染
    历史学期会报出**自信的错日期**（实测 `20242` 被写成「第 4 周（本周）9月21日 - 9月27日」）。
    判据只比**学年 + 上下半学期**，不猜具体日期；学期码形态不符时返回 true（拿不准就不改行为）。
    对不上 → 按「不知道」处理，文案说「切回当前学期即可」，**不是**「请去校准」（后者把人引错地方）。
- **两个「设置改了但页面没变」的教训（都踩过，都属于静默失效）**：
  1. **页面/VM 必须订阅偏好，不能只在加载时读一次**。课表页有「已是 Ready 就直接 return」的
     短路，锚点又在 `load()` 里读一次 → 校准页三处证据（界面/prefs/日志）全对，课表页纹丝不动。
     正确做法：VM 里订阅 `settings.prefs`，值变了**就地重算派生状态**（锚点与网络无关，
     不该为了重算再打一遍接口）。守卫只比 `monday` + `source`，**不比 `savedAt`**
     （写入每次都盖新时间戳，比它会让「自己刚写的值」被当成「用户又改了一次」白重算一遍）。
  2. **`Dispatchers.Main.immediate` 上写 StateFlow 会同步唤醒订阅者 → 嵌套重入写入**。
     `_prefs.value = next` 之后订阅者可能反手又写一次偏好，此时 `next` 已过期；
     若落盘写 `next` 就会把订阅者刚写的值覆盖回去。实测点「清除」后内存 `2026-08-31|exam`、
     磁盘被写成 null → 下次冷启动锚点凭空消失。**落盘一律写最新的 `_prefs.value`**。
     副作用：日志顺序会交错（嵌套的日志排在外层前面），不要据日志先后推断事件先后。
- **Kotlin 零警告的两个坑**：`"第$n周"` 里的「周」是合法标识符字符 → 被当成变量名（写 `${n}`）；
  智能转换会让 `!fitsTerm && x != null` 报 `Condition is always 'true'`
  （`fitsTerm` 的定义里已蕴含 `x != null`）→ 直接写 `!fitsTerm`。改完 grep `\p{Han}` 与 `^w:` 复查。
