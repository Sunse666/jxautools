...
## ⚠️ 引用批次号必须带前缀（2026-09-22 补）
项目里**同时存在两套 P0/P1/P2**：UI 改造的（`docs/UI改造实施大纲.md`）与 T2 的（`docs/T2功能实施大纲.md`）。
裸写「P0」一律是歧义 → **必须写「UI-P0」/「T2-P0」**。
...
- **交付 ≠ 编译**（2026-09-22 那次返工的根因）：`:app:compileDebugKotlin` **不产出 APK**，
  它只回答「源码能不能编过」。改了源码只跑 compile，`app-debug.apk` 会停在**旧时间戳**上 ——
  装机后看到的还是旧界面，而**编译零 `e:` 零 `w:`、全部静态验证全绿**，查无可查
  （用户只会说一句「这些都没编译？」）。**凡是要装给人看的一律 `:app:assembleDebug`**，
  装前核 APK 时间戳晚于 `git log -1 --format='%ci'`。
  **新旧包一眼判别**：看自检项数 —— 同一台设备上旧包 `520/520`、含 P3 的包 `654/654`。
- **真机验收清单 §4（P3 五条）已于 2026-09-22 在 MuMu 预跑全过**：判据是硬指标 ——
  顶栏折叠后标题**从 dump 消失**且最顶内容落到 y=56（≈状态栏高）、
  状态栏与顶栏两条竖列**逐像素同色 `#F8F9FC`**、子页顶栏滚两屏坐标**纹丝不动**、
  课表页轴 pitch 全 `134.0px` 列距全 `154.0px` 且 4 个像素脚本全 PASS。
- **UI 清单 §1/§2/§3（共 15 项）已于 2026-09-22 预跑**：13 项过、**2 项未跑** —— 2-1（访问通道三段）与
  2-3①（记住密码）：看登录页必须退出登录，而退出后可能要**人工输验证码**，风险不对等 → 没动登录状态。
  预跑记录已写进 `docs/UI改造真机验收清单.md`（§1/§2/§3 各一张表）。
- **真机验收的两件通用武器**（比肉眼盯 N 张图硬得多）：
  ① `tools/scan_theme_apply.py` —— 跨配置**不变像素**扫描：在 N 张主题截图里**每通道差 ≤2 且
  `max−min>40`（有饱和度）**的像素 = 没跟主题走的嫌疑。**「有饱和度」是关键条件** —— 不跟随的像素
  绝大多数本就该不变（白底黑字灰线，饱和≈0），一个**鲜艳**色在 12 个主题下纹丝不动才是硬编码或
  M3 baseline 泄漏（脚本会注解 baseline 紫的角色名）。退出码 1 = 有嫌疑。
  ② **偏移扫描** —— 整屏差异率会把「整体平移」误报成「内容真变了」，**别拿它当结论**。差异大时先扫
  dy 找零差异那一档（实证：字号 4 档整屏差 12.9%，dy=+9 时差异 **0.000%** → 只是顶部文字变高把课表推下去）。
  ③ **慢速变异包** —— 正常时长 ≤300ms 的过渡**抓不到连续轨迹**（连拍只捞到零星中间帧）。把动效时长
  ×8（260→2080ms）重装，连拍每张都是中间态且 dB 单调递减 60.40%→0.00%，铁证。凡「过程不可见」的
  验证都适用这招：放大时间尺度再采样。配套量化：`tools/measure_motion_transition.py`（与两个稳定
  基准帧的差异率对照，自带基准取错自证）。
- **动效已交付**（`38d4496` 代码 + `214cda8` 验收补录）：`ui/Motion.kt` 是全应用**唯一**动效令牌与
  出口（MotionSwap / MotionPager / motionHeight / motionItem），时长缓动只此一处。两个刻意不做：
  **滚动本身不加动画**（物理驱动叠过渡=粘手）、**主题/字号切换不加**（换皮加色彩流动像卡）。
  切 Tab 位置保留靠 `rememberSaveableStateHolder` + `SaveableStateProvider`（**变异实锤**：
  摘掉即丢）。验收清单新增 §5 动效 5 条（5/5 过）。
- **Gradle 在沙箱里会假死**：写 `app/build/intermediates` 被拦 → `dexBuilderDebug FAILED`，
  且错误被 grep 过滤器吞掉，看着像神秘构建失败。**dexBuilder 失败先怀疑沙箱，不是代码**
  （非沙箱重跑 28s 即过）。
...
## 协作方式（2026-09-22 起）
- **真机测试默认由用户执行**；但**经用户拍板的预跑批次例外**（如 UI 验收 §1-§5：用户选了
  「我像 P3 那样在模拟器上逐项跑完」），跑完把判据留进验收清单供复核。
  变异实验（改源码重装）只在预跑批次内做，**跑完必须还原并重建正式包**。
...
- **底部导航栏别按 `dump` 的 bounds 中心点**：它把系统 insets 算进去了 —— dump 报
  `'我的' (793,1552)`，而导航栏真正绘制在 **y=1440..1526**，按 1552 点**完全没反应**
  （表现得像「切不了 Tab」）。用 **y≈1483**。`input tap` 没反应时**先换 y 重试**再怀疑控件；
  **子页里底部导航是好用的**（子页属于「我的」，保持选中态是对的），别误判成被禁用。
