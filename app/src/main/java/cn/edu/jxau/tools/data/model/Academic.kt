...
data class CourseSlot(
    ...
    /** `Jxbbh` 教学班编号。课表里只是标识；Pro 版还把它当选课/退选的唯一凭据 */
    val classNo: String = "",
    /** `Jxb` 教学班名称 */
...
// 去抢课分支（2026-09-23）删掉了本文件里的 `CourseClass`（教学班）、`SelectionStats`（选课汇总）
// 与 `SelectionScope`（选课范围）三个模型，连同 `CourseClass.selfTest()` 那组「容量字段三态」用例。
// 容量语义的三条结论（`MaxRs==0` 是「服务端没给容量」不是「容量为零」、`Xkrl` 可为负、
// 「余量 <= 0 不等于不能选」）只对选课界面成立，代码与结论一并归档在 git 历史里。
...
data class ExamItem(
    ...
    val courseCode: String = "",
    /**
     * `Ksbname` —— **不是考场名称，是考试班名称**。
     *
     * ⚠️ 2026-09-21 真机核对时发现字段名骗人：实测值是
     * `线性代数A11220补考`（课程名 + 课程号 + 类别拼起来的），
     * 而真正的考试地点在 [place]（`Ksdd`，值是 `5-130（南楼）,E-104`）。
     * 早期注释写成「考场名称」，照着显示就成了「考场：5-130（南楼）　线性代数A11220补考」——
     * 把课程名又念了一遍。展示时只用 [place]。
     */
    val roomName: String = "",
...
// 去抢课分支（2026-09-23）删掉了 `TicketCheckState`（`User/CheckGuid` 的票据校验结果）
// 与 `WriteResult`（选课 / 退选的写操作回执）。两者都随选课下线：前者唯一调用方是
// `JwglApi.checkTicket()`（也已被删，且它本来就没有调用方），后者只被选课的 `write()` 外壳使用。
// ⚠️ 这两块各有一条**值得记住的正确结论**，别随代码一起忘掉：
//   · `User/CheckGuid` **不是「选课窗口开关」**（我先前的结论是错的）——它答的是
//     「这个 guid 的票据还有效吗」，拿它当窗口开关会两头出错：窗口关着时显示「已开放」，
//     票据过期时反而显示「未开放」。窗口状态教务系统没有可查接口，只能从课表/回执间接推断。
//   · 写操作的**成功响应没有实测样本**（选课窗口关闭期间打不通），所以「没见到 `Result:true`」
//     不能直接当失败 —— 那是三态（成功 / 失败 / 判不出来），不是布尔；误报比不报更糟。
