#!/usr/bin/env bash
# 过渡动画的验证套件：静态对账 + 变异探针，外面再包一层独立进程的 md5 复核。
#
# 为什么外面还要包一层：`probe_motion.py` 自己在进程内做了「改前/改后 md5 一致」的判断，
# 但 `tools/kotlin-check/probe.py` 出过一次事故 —— 写回没落盘，同进程读回来却是新值，
# **变异外溢而汇报是绿的**。所以这里再另起进程用 `md5sum` 判一次，
# 两次结论不一致就报错（宁可报错也不要假通过）。
#
# 跑法：bash tools/probe_motion.sh   （可用 PYTHON=/path/to/python 覆盖解释器）
...
echo "===== 1/2 静态对账（真实源码）====="
"$PYTHON" "$ROOT/tools/verify_motion.py"
RC_CHECK=$?

echo
echo "===== 2/2 变异探针（只改副本）====="
"$PYTHON" "$ROOT/tools/probe_motion.py"
RC_PROBE=$?
